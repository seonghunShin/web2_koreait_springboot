...
-- 여러 행 -> 한 숫자로
select
	count(*) as rowCount, -- count(컬럼명 or *)
...
-- count(컬럼)은 null 카운팅 하지 x

# round - 소수를 다룰 때
select
	round(avg(price)) as 정수, -- 정수로 반올림
    round(avg(price), 1) as 소수한자리, -- 두번째 매개변수가 양수면 소수자리표현
...
-- 위에서 아래로 평가되며, 처음 true가 된곳에서 실행하고 탈출
select
	product_name,
    price,
    case -- if문시작
...
-- 3) post테이블에서 제목, 작성시각, 시간대(오전(0~12),오후(13~18),밤(19~24))
-- date_format으로 시간만 추출해서 대소비교
select
	title,
    create_at,
    case
		when date_format(create_at, '%H') <= 12 then '오전'
		when date_format(create_at, '%H') <= 18 then '오전'
        else '밤'
    end as 시간대
from
	post
