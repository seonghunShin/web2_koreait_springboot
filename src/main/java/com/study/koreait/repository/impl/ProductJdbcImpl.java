...
public class ProductJdbcImpl implements ProductRepository {
    ...
    // spring-boot-starter-jdbc 라이브러리가 빈등록까지 해 줌
    ...
    // 자원반납을 하는 메서드
    // 매개변수로 들어오는 타입은 같은데, 실제 동작이 각각 다른것 -> 다형성
    ...
    public Product findProductById(int id) {
        // sql injection 방어를 위해 미리 준비되어있는 메서드를 사용한다.
    ...
    public int deleteProductById(int id) {
        ...
        try {
            ...
            // 조건문 안걸릴때만 리턴 됨
            // 조건문에 걸리면 예외전파되서 리턴안됨
            return successCount;

        } catch (SQLException e) {
            log.error("deleteProductById 실패 - id={}", id, e);
    ...
    public int updateProductById(Product product) {
        ...
        try {
            ...
            int successCount = ps.executeUpdate();
            if(successCount == 0) {
                throw new ProductException("해당 id는 존재하지 않는 상품입니다", HttpStatus.BAD_REQUEST);
            }
            return successCount;

        } catch (SQLException e) {
            log.error("updateProductById 실패 - {}", product, e);
            throw new ProductException("상품 업데이트 중 오류가 발생했습니다", HttpStatus.INTERNAL_SERVER_ERROR);
        } finally {
            close(ps); close(conn);
        }
