...
public class SecurityConfig { // 시큐리티 관련 설정만
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.sessionManagement(session ->
                session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)); // 세션 안만들 것
        ...
        http.authorizeHttpRequests(auth -> {
            ...
            */
            ...
            auth.requestMatchers("/post/**", "/auth/**")
                    .permitAll()
                    .anyRequest().authenticated(); // 모든 요청은 인가받으시오

        });

        // UsernamePasswordAuthenticationFilter라는 필터가 있는데 그전에 JwtAuthenticationFilter라는 필터를 끼워넣어 달라는 뜻.
